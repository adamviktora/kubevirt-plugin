...
test.describe('Create template button', () => {
  ...
  test('"From an existing template" opens Clone modal', async ({ page, templatesPage }) => {
    ...
    await expect(byTest(page, 'save-button')).toBeDisabled();
  ...
  test('"With YAML" opens YAML editor', async ({ page, templatesPage }) => {
    await templatesPage.selectCreateOption('With YAML');

    await expect(page).toHaveURL(/\/~new$/, { timeout: NAV_TIMEOUT });
    await expect(page.getByText('name: example')).toBeVisible({ timeout: NAV_TIMEOUT });
  });
});
